...
void Pila::push(char elemento) {
    if (llena()) {
        cout << "Pila llena, por favor de borrar algun dato...";
        pausa();
    }
    else {
        agrega(elemento);
...
void Pila::pop () {
    if (vacia()){
        std::cout << std::endl << "Pila vacia, por favor ingresar elementos...";
        pausa();
    }
    else {
        ultMenos();
...
string conversor(string infija) {
    ...
    for (int i = 0; i < (int)infija.length(); i++) {
        ...
        ///Si el caracter es un parentesis de cierre.
            ...
            pila.pop(); ///Desapila el parentesis de apertura.
        }
...
int main () {
    ...
    do {
        ...
        cout << "1)._ Convertir de notacion infija a posfija." <<endl;
        cout << "0)._ Salir." <<endl;
        cin >> opc;
